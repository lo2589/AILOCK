...
from ailatch.format import MAGIC, encode_file, is_locked, parse_locked_file
from ailatch.runner import AILatchIOPatch, run_in_memory
...
class EncryptedWritebackTests(unittest.TestCase):
    ...
    def test_text_write_is_reencrypted_on_close(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-writeback-") as temp_dir:
            root = Path(temp_dir)
    ...
    def test_rplus_persists_the_modified_plaintext(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-rplus-") as temp_dir:
            root = Path(temp_dir)
            target = root / "secret.txt"
    ...
    def test_append_writes_at_end_even_after_seek(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-append-") as temp_dir:
            root = Path(temp_dir)
            target = root / "secret.txt"
            target.write_bytes(_locked_blob())
    ...
    def test_path_write_text_uses_encrypted_writeback(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-pathlib-") as temp_dir:
            root = Path(temp_dir)
            target = root / "secret.txt"
            target.write_bytes(_locked_blob())
            patch, commits = self._patch(root)
    ...
    def test_io_open_uses_encrypted_writeback(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-io-open-") as temp_dir:
            root = Path(temp_dir)
    ...
    def test_write_is_blocked_when_reencryption_is_unavailable(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-block-write-") as temp_dir:
            root = Path(temp_dir)
            target = root / "secret.txt"
            original_blob = _locked_blob()
            target.write_bytes(original_blob)
            patch = AILatchIOPatch([root], decrypt_fn=lambda blob: b"original")
    ...
    def test_v2_reencryption_preserves_recovery_wrap(self):
        fake_crypto = types.ModuleType("ailatch.crypto")
    ...
    def test_run_in_memory_wires_encrypted_writeback(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-run-writeback-") as temp_dir:
            root = Path(temp_dir)
            entry = root / "main.py"
            target = root / "secret.txt"
            entry.write_text(
    ...
    def test_invalid_encrypted_result_never_replaces_original(self):
        with tempfile.TemporaryDirectory(prefix="ailatch-invalid-encrypt-") as temp_dir:
            root = Path(temp_dir)
            target = root / "secret.txt"
            original_blob = _locked_blob()
            target.write_bytes(original_blob)
